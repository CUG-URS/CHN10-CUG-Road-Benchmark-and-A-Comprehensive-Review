...
class DeeplabV3_plus(BasicModule):
    ''' Main model: DeepLabV3+ '''
    def __init__(self, num_classes=1,  # Change to 1 for binary segmentation
                 resnet_arch='resnet50', output_stride=8, layer_num=2,
                 aspp_rate=0):
        super().__init__()
        self.model_name = 'deeplabv3plus'
        ...
        self.conv2 = self._make_layer(64, 48, 1)

        rate_table = [1, 6, 12, 18, 24, 1, 3]
        if aspp_rate == 0:
            self.aspp = ASPP(
                in_channel=layers_dim[layer_num - 1],
                depth=aspp_depth)
        else:
            self.aspp = ASPP_test(layers_dim[layer_num - 1], aspp_depth, rate_table[aspp_rate])

        # Decoder
        self.decoder_conv1 = self._make_layer(aspp_depth + 48, aspp_depth, 3, padding=1)
        self.decoder_conv2 = self._make_layer(aspp_depth, aspp_depth, 3, padding=1)

        self.out_conv = nn.Conv2d(aspp_depth, 1, 1, 1)  # Output with 1 channel
    ...
    def forward(self, x):
        ...
        if list(x.shape[2:]) != size_8:
            x = resize(x, newsize=size_8)
        x = self.aspp(x)

        decoder_features = resize(x, newsize=pool2.shape[2:])
        encoder_features = self.conv2(pool2)
        x = torch.cat([encoder_features, decoder_features], dim=1)
        x = self.decoder_conv1(x)
        x = self.decoder_conv2(x)
        x = resize(x, newsize=size)
