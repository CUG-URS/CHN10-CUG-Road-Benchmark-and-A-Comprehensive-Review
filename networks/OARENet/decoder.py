...
class Decoder(nn.Module):
    def __init__(self, filters=[256, 512, 1024, 2048]):
        ...
        self.conv_e3 = nn.Sequential(
            nn.Conv2d(filters[2], filters[0], 1, bias=False),
            nn.BatchNorm2d(filters[0]),
            nn.ReLU()
        )

        self.conv_e2 = nn.Sequential(
            nn.Conv2d(filters[1], int(filters[0] / 2), 1, bias=False),
            nn.BatchNorm2d(int(filters[0] / 2)),
            nn.ReLU()
        )

        self.conv_e1 = nn.Sequential(
            nn.Conv2d(filters[0], int(filters[0] / 4), 1, bias=False),
    ...
    def forward(self, e1, e2, e3, e4):
        # Adjust the size of e3 to match decoder4(e4) before concatenation
        d4 = self.decoder4(e4)
        conv_e3 = self.conv_e3(e3)
        if d4.size(2) != conv_e3.size(2) or d4.size(3) != conv_e3.size(3):
            conv_e3 = F.interpolate(conv_e3, size=(d4.size(2), d4.size(3)), mode='bilinear', align_corners=False)
        d4 = torch.cat((d4, conv_e3), dim=1)

        # Adjust the size of e2 to match decoder3(d4) before concatenation
        d3 = self.decoder3(d4)
        conv_e2 = self.conv_e2(e2)
        if d3.size(2) != conv_e2.size(2) or d3.size(3) != conv_e2.size(3):
            conv_e2 = F.interpolate(conv_e2, size=(d3.size(2), d3.size(3)), mode='bilinear', align_corners=False)
        d3 = torch.cat((d3, conv_e2), dim=1)

        # Adjust the size of e1 to match decoder2(d3) before concatenation
        d2 = self.decoder2(d3)
        conv_e1 = self.conv_e1(e1)
        if d2.size(2) != conv_e1.size(2) or d2.size(3) != conv_e1.size(3):
            conv_e1 = F.interpolate(conv_e1, size=(d2.size(2), d2.size(3)), mode='bilinear', align_corners=False)
        d2 = torch.cat((d2, conv_e1), dim=1)
...
class Decoder2(nn.Module):
    ...
    def forward(self, e1, e2, e3, e4):
        # Ensure matching sizes before element-wise addition
        d4 = self.decoder4(e4)
        conv_e3 = self.conv_e3(e3)
        if d4.size(2) != conv_e3.size(2) or d4.size(3) != conv_e3.size(3):
            conv_e3 = F.interpolate(conv_e3, size=(d4.size(2), d4.size(3)), mode='bilinear', align_corners=False)
