...
# from net.A8_CoANet.model.sync_batchnorm.batchnorm import SynchronizedBatchNorm2d
...
import torch.nn as nn
import torch.nn.functional as F
from .CoANet.model.sync_batchnorm.batchnorm import SynchronizedBatchNorm2d
...
class Decoder(nn.Module):
    ...
    #     # Adjusting the size using F.interpolate to ensure dimensions match
    #     d3 = F.interpolate(d3, size=(e2.size(2), e2.size(3)), mode='bilinear', align_corners=True)
        
    #     d2 = torch.cat((self.decoder2(d3), self.conv_e1(e1)), dim=1)

    #     # Adjusting size here as well
    ...
    def forward(self, e1, e2, e3, e4):
        ...
        # Decoder3 stage
        # Adjust size of self.decoder3(d4) to match e2 size
        d3 = self.decoder3(d4)
        d3 = F.interpolate(d3, size=(e2.size(2), e2.size(3)), mode='bilinear', align_corners=True)
        
        d3 = torch.cat((d3, self.conv_e2(e2)), dim=1)
        
        # Decoder2 stage
        d2 = self.decoder2(d3)
        d2 = F.interpolate(d2, size=(e1.size(2), e1.size(3)), mode='bilinear', align_corners=True)
        
        d2 = torch.cat((d2, self.conv_e1(e1)), dim=1)
