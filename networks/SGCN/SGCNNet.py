...
# url：https://github.com/tist0bsc/SGCN
# paper: Split Depth-wise Separable Graph Convolution Network for Road Extraction in Complex Environment from High-resolution Remote Sensing Imagery
...
class Sobel(nn.Module):   
    def __init__(self,in_channel,out_channel):
        ...
        kernel_x = kernel_x.type(torch.FloatTensor)
        kernel_y = torch.cuda.FloatTensor(kernel_y).expand(out_channel,in_channel,3,3)
        kernel_y = kernel_y.type(torch.FloatTensor)
    ...
    def forward(self,x):
        b,c,h,w = x.size()
        sobel_x = F.conv2d(x,self.weight_x.cuda(),stride=1, padding=1)
        sobel_x = torch.abs(sobel_x)
        sobel_y = F.conv2d(x,self.weight_y.cuda(),stride=1, padding=1)
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.last_activation(final)
...
if __name__ == '__main__':
    ...
    class_num = 1
    model = SGCN_res50(num_classes=class_num).cuda()
